...
public class ArmorView extends SearchView<Armor> {
    ...
    public ArmorView(IView parent) {
        ...
        Database.armorList.addListener((observable, o, n) -> update());
    ...
    private void search() {
        for (int i = 0; i < searchCount.intValue(); i++) {
            ...
            Stream<Armor> stream = Database.armorList.stream().filter(w -> w.getRarity().equals(rarity));
    ...
    private void update() {
        if (!Database.armorList.isEmpty()) {
            ...
            for (Armor armor : Database.armorList) {
