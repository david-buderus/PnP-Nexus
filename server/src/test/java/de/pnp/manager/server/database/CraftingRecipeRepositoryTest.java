...
class CraftingRecipeRepositoryTest extends
    ...
    @Test
    void testResultLink() {
        Item resultOld = itemRepository.insert(getUniverseId(),
                createItem().withName("Result old").buildItem());
        Item resultNew = createItem().withName("Result new").buildItem();
        Item materialItem = itemRepository.insert(getUniverseId(),
    ...
    void testCraftingRecipeMaterial() {
        Item result = itemRepository.insert(getUniverseId(),
        ...
        repository.insert(getUniverseId(), craftingRecipe);
        assertThat(repository.getAll(getUniverseId())).contains(craftingRecipe);
    }
    ...
    protected CraftingRecipe createObject() {
        Item result = itemRepository.insert(getUniverseId(),
                createItem().withName("Result").buildItem());
        Item itemA = itemRepository.insert(getUniverseId(),
                createItem().withName("A").buildItem());
        Item itemB = itemRepository.insert(getUniverseId(),
    ...
    protected CraftingRecipe createSlightlyChangeObject() {
        Item result = itemRepository.get(getUniverseId(), "Result").orElseThrow();
        Item itemA = itemRepository.get(getUniverseId(), "A").orElseThrow();
        Item itemB = itemRepository.get(getUniverseId(), "B").orElseThrow();
