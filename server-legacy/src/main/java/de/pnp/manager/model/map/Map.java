...
import de.pnp.manager.model.map.object.MapObjectMap;
...
import de.pnp.manager.model.map.object.room.room.Entrance;
...
public class Map implements SeededRandom {
    ...
    protected void generateLootObjects() {
        ...
        for (RoomObject roomObject : roomMap.getAllMapObjects()) {
            if (roomObject instanceof WithLootables) {
                for (LootObject lootObject : ((WithLootables) roomObject).generateLootables(specification)) {
