package de.pnp.manager.model.character.generation;
...
public class Drop {
    ...
    public void addToLootTable(LootTable lootTable, PnPCharacter character) {
        if (character.getLevel() >= minLevel && character.getLevel() <= maxLevel &&
                character.getTier() >= minTier && character.getTier() <= maxTier) {
            float amount = this.multiplicativeAmount;

            if (levelMultiplication != 0) {
                amount *= levelMultiplication * character.getLevel();
            }
            if (tierMultiplication != 0) {
                amount *= tierMultiplication * character.getTier();
            }
