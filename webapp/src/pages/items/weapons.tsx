...
import { ERarity, ItemServiceApi, Weapon } from '../../api';
...
const WeaponPage = () => {
    ...
    return <OverviewBasePage<Weapon>
        ...
        fetchObjects={universe => ITEM_API.getAllWeapons(universe)}
        removeObjects={(universe, selected) => ITEM_API.deleteAllItems(universe, selected)}
        editObject={(universe, id, itemType) => ITEM_API.updateItem(universe, id, itemType).then(mapResponse<Weapon>)}
        createObjects={(universe, objs) => ITEM_API.insertAllItems(universe, objs).then(mapArrayResponse<Weapon>)}
        emptyObject={{
            "@type": "Weapon",
            name: "",
            tags: new Set<string>(),
